...
import { Checkbox as CheckboxPrimitive, type CheckboxProps as CheckboxPrimitiveProps } from '@headlessui/react';
...
type CheckboxProps<TTag extends React.ElementType = typeof motion.button> = CheckboxPrimitiveProps<TTag> &
  Omit<HTMLMotionProps<'button'>, 'checked' | 'onChange' | 'defaultChecked' | 'children'> & {
    as?: TTag;
  };

function Checkbox<TTag extends React.ElementType = typeof motion.button>({ children, ...props }: CheckboxProps<TTag>) {
  const { as = motion.button, ...rest } = props;

  return (
    <CheckboxPrimitive data-slot='checkbox' whileTap={{ scale: 0.95 }} whileHover={{ scale: 1.05 }} {...rest} as={as as React.ElementType}>
      {(bag) => (
